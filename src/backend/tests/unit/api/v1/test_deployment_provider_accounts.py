...
def _provider_payload(
    *,
    account_id: str = "tenant-1",
    provider_key: str = "watsonx-orchestrate",
...
async def test_deployment_provider_account_crud(client: AsyncClient, logged_in_headers):
    ...
    list_response = await client.get("api/v1/deployment-providers/", headers=logged_in_headers)
    assert list_response.status_code == status.HTTP_200_OK
    listed = list_response.json()["deployment_providers"]
    assert any(item["id"] == created["id"] for item in listed)

    get_response = await client.get(f"api/v1/deployment-providers/{created['id']}", headers=logged_in_headers)
    assert get_response.status_code == status.HTTP_200_OK
    assert get_response.json()["id"] == created["id"]

    update_response = await client.patch(
        f"api/v1/deployment-providers/{created['id']}",
    ...
    delete_response = await client.delete(f"api/v1/deployment-providers/{created['id']}", headers=logged_in_headers)
    assert delete_response.status_code == status.HTTP_204_NO_CONTENT

    not_found_response = await client.get(f"api/v1/deployment-providers/{created['id']}", headers=logged_in_headers)
...
async def test_deployment_provider_account_is_user_scoped(
    ...
    other_user_get = await client.get(f"api/v1/deployment-providers/{created['id']}", headers=other_user_headers)
